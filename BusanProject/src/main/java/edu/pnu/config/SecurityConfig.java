...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(security -> security
				.requestMatchers("/comment/add", "/comment/edit", "/comment/delete").authenticated()
				.requestMatchers("/admin/**").hasRole("ADMIN")
				.anyRequest().permitAll());

		// http.authorizeHttpRequests(security -> security
		// .requestMatchers("/comment/**").permitAll() // 댓글 경로에 대해서는 누구나 접근 허용
		// .requestMatchers("/admin/**").hasRole("ADMIN") // 관리자 경로에 대해서는 ADMIN 역할이 있는
		// 사용자만 접근 허용
		...
		http.addFilterBefore(new JWTAuthorizationFilter(busanUserRepository, secretKey),
				UsernamePasswordAuthenticationFilter.class); // 권한검사
		return http.build();
	}
}
